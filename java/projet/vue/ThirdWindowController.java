...
/**
 * every method used for the third view (the front view)
 */
public class ThirdWindowController implements Observer{
	/**
	 * the current canvas
	 */
	@FXML Canvas canvas;
    ...
    /**
     * sets the window's model 
     * @param obj the 3d model shown
     */
	...
	public void update(Subject subj) {
		...
		for (int i = 0; i < pts.size(); i++) {
			double[][] contenu = new double[][] {{pts.get(i).getX()},
				...
				Matrice res = new Matrice(GeometryAffine.rotationY.getMatrice().multiplication(matrice).getContenu());
